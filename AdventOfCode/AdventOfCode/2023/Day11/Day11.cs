namespace AdventOfCode._2023.Day11
{
    public class Day11
    {
        public long Run(string input, int expansion = 1)
        {
            ...
            for (var i = 0; i < galaxies.Count(); i++)
            {
                for (var j = i + 1; j < galaxies.Count(); j++)
                {
                    ...
                    long expandedRowsToAdd = expandedRows.Count(rowIndex => rowIndex > Math.Min(first.Y, second.Y) && rowIndex < Math.Max(first.Y, second.Y));
                    long expandedColsToAdd = expandedColumns.Count(colIndex => colIndex > Math.Min(first.X, second.X) && colIndex < Math.Max(first.X, second.X));
                    overallDistance += expansion * (expandedRowsToAdd + expandedColsToAdd);
                }
            }
